...
const register = async () => {
  try {
    const { registerUser, error } = useAppwriteUsers();
...
<template>
  <section class="w-full h-screen flex justify-start items-center flex-col">
    <section class="mt-20 w-10/12 flex justify-start items-start flex-col">
      <h1 class="mb-10 text-9xl font-general">Registrarse</h1>
      <v-text-field
        label="Email"
        variant="outlined"
        type="email"
      ...
      <v-text-field
        ...
        class="w-6/12"
      ></v-text-field>
      <v-btn variant="outlined" class="my-4" type="submit" @click="register()">
        Acceder
      </v-btn>
    </section>
  </section>
